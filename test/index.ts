...
describe('node-module-resolution', () => {
  ...
  it('resolves node_modules at the same level as a single file module',()=>{
    ...
    const nmr = new NodeModuleResolution(files);
    const result = nmr.resolve('b',{id:'/node_modules/borp/node_modules/a.js',paths:[]})
  ...
  it("resolves from parents node_modules",()=>{
    const files = new Map([
      file('/node_modules/b/index.js', 'b index')
    ]);

    const nmr = new NodeModuleResolution(files);
    const result = nmr.resolve('b',{id:'/node_modules/borp/index.js',paths:[]})
...
const file = (path: string, content?: string, realpath?: string):
    [string, FileObject] => {
      let file = {getData: () => content ? Buffer.from(content) : undefined} as FileObject;
